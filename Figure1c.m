...
%%% find rats with at least 120000 trials
...
for i=1:length(rat_list)
    ...
    %%% select the matrix of %right choices for each condition
    ...
    %%% generative gamma for each condition
    vec=[-4 -2.5 -1 1 2.5 4];
    ...
    subplot(2,2,1)
    hold on    
    %%% compute mean across frequency strengths
    y1=nanmean(le1,1);
    ylim([0 1])
    xlim([-4 4])
    hold on
    ...
    set(gca,'TickDir','out')
    set(gca,'FontSize',20);
    %%% compute fit of psychometrics curve
    ...
    subplot(2,2,2)
    hold on
    %%% compute mean across location strengths
    y2=nanmean(le1,2);
    ylim([0 1])
    xlim([-4 4])
    hold on
